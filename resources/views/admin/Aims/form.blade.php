...
<div class="content-wrapper">
    ...
    <div class="content-header">
        <div class="container-fluid">
            <div class="row mb-2">
                <div class="col-sm-6">
                    <h1 class="m-0">Create Our Aims</h1>
                </div><!-- /.col -->
                <div class="col-sm-6">
                    <ol class="breadcrumb float-sm-right">
                        <li class="breadcrumb-item"><a href="{{ route('admin.dashboard') }}">Home</a></li>
                        <li class="breadcrumb-item"><a href="{{ URL('admin/aims') }}">Aims</a></li>
                        <li class="breadcrumb-item active">Create Aims</li>
                    </ol>
                </div><!-- /.col -->
            </div><!-- /.row -->
        </div><!-- /.container-fluid -->
                ...
                <div class="col-md-12">
                    ...
                    <div class="card card-primary">
                        <div class="card-header">
                            <h3 class="card-title">Aims Form</h3>
                        </div>
                        <!-- /.card-header -->
                        <div class="card-body">
                            <form  action="{{ URL('admin/aims') }}" enctype="multipart/form-data" method="POST">
                                {!! csrf_field() !!}
                                ...
                                $content = (array)json_decode($row->title);
                                }

                                @endphp
                                @foreach($languages as $lang)
                                <div class="form-group row">
                                    <label class="col-sm-2 col-form-label">Aim title in {{ $lang->name }}</label>
                                    <div class="col-sm-6">
                                        <input type="text" class="form-control" placeholder="Enter Aim title" name="title[{{ $lang->short_name }}]" value="{{ isset($title[$lang->short_name]) ? $title[$lang->short_name] : '' }}">
                                    </div>
                                </div>
                                @endforeach
                                @foreach($languages as $lang)
                                <div class="form-group row">
                                    <label class="col-sm-2 col-form-label">Aim Content in {{$lang->name}} </label>
                                    <div class="col-sm-6">
                                        <textarea id="summernote" name="content[{{$lang->short_name}}]" required>{{isset($content[$lang->short_name]) ? $content[$lang->short_name] : '' }}</textarea>
                                    </div>
                                </div>
                                @endforeach
                                ...
                                @php $tImage = asset($row->file); @endphp
                                <div class="form-group row">
                                    <label class="col-sm-2 col-form-label">Image</label>
                                    <div class="col-sm-6">
                                        <img src="{{ $tImage }}" width="50px" height="50px" />
                                    </div>
                                </div>
                                @endif
                                ...
                                <div class="form-group text-right">
                                    <div class="col-sm-12">
                                        <a href="{{ URL('admin/aims') }}" class="btn btn-info mr-2"> Cancel </a>
